...
from dark_factory.controller import Controller, validate_graph, default_bootstrap_graph
...
def test_full_loop_archive_before_cleanup(tmp_path: Path) -> None:
    ...
    ids = ctrl.intake_issues(
        [{"repo": "yeutterg/dark-factory", "number": 1, "title": "Demo", "body": "Do the thing", "is_root": True}]
    )
    wid = ids[0]
    ...
    item = store.one("SELECT status, presentation_stage FROM work_items WHERE id=?", (wid,))
...
def test_pause_blocks_dispatch(tmp_path: Path) -> None:
    ...
    ctrl = Controller(cfg, store, Path("src/dark_factory/agents/prompts"))
    ids = ctrl.intake_issues(
        [{"repo": "yeutterg/dark-factory", "number": 2, "title": "Paused", "body": "", "is_root": True}]
    )
...
def test_cancel_does_not_fallback(tmp_path: Path) -> None:
    ...
    ctrl = Controller(cfg, store, Path("src/dark_factory/agents/prompts"))
    ids = ctrl.intake_issues(
        [{"repo": "yeutterg/dark-factory", "number": 3, "title": "Cancel", "body": "", "is_root": True}]
    )
    ctrl.start_planning(ids[0])
    job = store.one("SELECT id FROM jobs WHERE kind='plan'")
    ctrl.cancel_job(job["id"], "tester")
    worker = Worker(cfg, store, ctrl, "t")
    assert worker.run_once() is False or store.one("SELECT status FROM jobs WHERE id=?", (job["id"],))["status"] == "cancelled"
    ...
    script.write_text(
        "\n".join(
            [
                ...
                "print(json.dumps({'ok': True, 'operation': req['operation'], 'evidence': {'alt': True}, 'items': [{'external_id': 'alt:1', 'url': '', 'title': 'alt', 'body': '', 'repo': 'yeutterg/dark-factory', 'parent_id': None, 'is_root': True, 'iteration': None}]}))",
            ]
        ),
        encoding="utf-8",
    )
...
def test_prompt_override(tmp_path: Path) -> None:
    ...
    text = Path("examples/factory.toml").read_text(encoding="utf-8")
    text = text.replace('state_dir = "./state"', f'state_dir = "{tmp_path / "state"}"')
    text = text.replace("# omit roles to use packaged defaults", f'planner = "{override}"')
    cfg = parse_toml_bytes(text.encode(), Path("examples/factory.toml").resolve())
