...
Stage every required plan, diff, fixture and evidence summary inside the assignment workspace before starting Claude. In restricted mode, its file tools may refuse a path outside the working directory even when the OS sandbox grants that individual file read access. Use workspace-relative paths in the prompt and verify the staged files and hashes before dispatch. A critique that could not read its target is a failed gate, not a successful review of the target; retain its diagnostic and retry in a fresh workspace with the required inputs.

For a repair review, freeze the candidate source and label every supplied check with its source tree, command, exit status and artifact hash. Earlier failures may explain the repair, but must be marked historical; earlier passes do not qualify changed source. If current checks are still running, ask for a source review and record execution evidence as pending. Attach their final results only after verifying the same inputs. Keep a finding disposition for each repair and exercise the actual caller that exposed the defect; an isolated helper pass does not establish dispatch order, report delivery or cross-repository acceptance.
...
Choose primary and fallback models through private deployment configuration. The [Fable plan documentation](https://support.claude.com/en/articles/15424964-claude-fable-models-on-your-plan) describes included Max usage and credit overage. [Claude Code model configuration](https://code.claude.com/docs/en/model-config) explains that unattended Fable requests can bill credits without a consent prompt. Subscription credentials alone therefore do not enforce a zero-overage policy. Verify account-level controls before a subscription-only run.

Live connection troubleshooting: macOS network-enabled attempts grant the system DNS resolver socket (`/private/var/run/mDNSResponder`) as well as outbound TCP. Network-disabled jobs receive neither grant. Native synthetic API failures remain failures and preserve the provider diagnostic instead of being misreported only as a model mismatch. A local `auth status` success establishes credential selection, not server-side token validity. When capturing `setup-token` privately, use a wide terminal and preserve the entire token; terminal line wrapping can otherwise truncate it. Never print the captured credential in logs or chat.
