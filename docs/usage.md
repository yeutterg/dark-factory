...
The repository includes a generic Codex `Stop` hook template in `examples/codex-hooks.json`. Copy it to `.codex/hooks.json` to enable it locally (`mkdir -p .codex && cp examples/codex-hooks.json .codex/hooks.json`). Both `.codex/` and `.claude/` directories are ignored and excluded from automatic publishing, including nested copies. It commits development changes and pushes the current branch to `yeutterg/dark-factory`, then verifies the remote commit. It excludes local profiles, credentials, authentication/session stores, state and generated artifacts; it refuses unfinished Git operations and never force-pushes. Before committing it checks the complete index for known credential patterns and machine-local paths. Before pushing it also checks unpublished commit trees, including prior commits in a clean checkout. The checks report categories without printing matched values. These checks supplement a manual privacy review; they cannot identify every kind of private information. New source, tests and documentation are included. Run relevant tests before ending a turn.

On a new installation, review and trust the hook through Codex `/hooks`. This is the normal [Codex hook trust flow](https://learn.chatgpt.com/docs/hooks#review-and-trust-hooks). The hook checkpoints code; it does not merge, accept an outcome or upgrade a controller.
